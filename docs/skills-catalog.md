...
| `multi-harness` | Cross-Harness Delegation | Delegate bounded subtasks to local harnesses (Pi, OpenCode, Codex CLI, Claude Code CLI, or mcode via Herdr) with prompt isolation and output contracts. | cross-harness requests, comparing harnesses, external-only runtimes |
