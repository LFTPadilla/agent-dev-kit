...
Spawn **one** `general-purpose` subagent with `isolation: "worktree"`. Executor model: default frontier executor (`opus-5`, `gpt-5.6-sol`, `gemini-3.7-flash (high)`); use what the user named if they named one (`execute 003 fable-5`).
